...
class Solana_Simplified:
    ...
    def verify_token_account(spl_client: Token, token_address_publickey: Pubkey):
        try:
            spl_client.get_account_info(
                account=token_address_publickey, commitment=None
            )
            return True
        except:
    ...
    def get_main_wallet_solana_balance(
        solana_client: Client, sender_main_wallet_public_key: Pubkey
    ):
        solana_balance = float(
            solana_client.get_balance(sender_main_wallet_public_key).value
        ) / float(1000000000)
        return solana_balance

    def get_token_account_balance(spl_client: Token, token_address_publickey: Pubkey):
        token_balance = spl_client.get_balance(
            token_address_publickey
        ).value.ui_amount_string
        return token_balance

    def approve_spl_token_transaction(
        spl_client: Token,
        source_token_address_publickey: Pubkey,
        delegate_token_address_publickey: Pubkey,
        owner_token_address_publickey: Pubkey,
        amount: float,
    ):
        amount = int(amount * 1000000000)
        transaction_signature = spl_client.approve(
            source_token_address_publickey,
            delegate_token_address_publickey,
            owner_token_address_publickey,
            amount,
            multi_signers=None,
            opts=None,
            recent_blockhash=None,
    ...
    def send_spl_token(
        spl_client: Token,
        source_token_address_publickey: Pubkey,
        destination_token_address_publickey: Pubkey,
        source_main_wallet_keypair: Keypair,
        amount: float,
    ):
        transaction_signature = spl_client.transfer(
            source=source_token_address_publickey,
    ...
    def send_solana(
        solana_client: Client,
        sender_main_wallet_public_key: Pubkey,
        destination_main_wallet_public_key: Pubkey,
        source_main_wallet_keypair: Keypair,
        amount: float,
    ):
        txn = Transaction().add(
            transfer(
                TransferParams(
                    from_pubkey=sender_main_wallet_public_key,
                    to_pubkey=destination_main_wallet_public_key,
                    lamports=int(amount * 1000000000),
                )
            )
        )
        transaction_signature = solana_client.send_transaction(
            txn, source_main_wallet_keypair
        ).value
        return str(transaction_signature)
    ...
    def check_solana_transaction(
        solana_client: Client, transaction_signature: Signature
    ):
        transaction_complete = solana_client.get_transaction(
    ...
    def check_approval_transaction(
        solana_client: Client, transaction_signature: Signature
    ):
        transaction_complete = solana_client.get_transaction(
            tx_sig=transaction_signature,
    ...
    def check_token_transaction(
        ...
        initial_balance = (
            transaction_complete.value.transaction.meta.pre_token_balances[
                0
            ].ui_token_amount.ui_amount_string
        )
        if prior_balance == initial_balance:
    ...
    def check_solana_transaction_direct(
        solana_client: Client, transaction_signature: Signature
    ):
        transaction_status = (
    ...
    def get_transaction_signature_list(solana_client: Client, owner_account: Pubkey):
        signatures = solana_client.get_signatures_for_address(
            owner_account, before=None, until=None, limit=None, commitment=None
        ).value
        signatures_list = [signature.signature for signature in signatures]
        return signatures_list

    def get_transaction_details_from_signature(
        solana_client: Client, transaction_signature: Signature
    ):
        transaction_complete = solana_client.get_transaction(
            tx_sig=transaction_signature,
    ...
    def check_transaction_status(
        solana_client: Client, sender_account: Pubkey, transaction_complete
    ):
        try:
            prior_balance = (
                transaction_complete.value.transaction.meta.post_token_balances[
                    0
                ].ui_token_amount.ui_amount_string
            )
            initial_balance = (
            ...
            token_account = (
                transaction_complete.value.transaction.transaction.message.account_keys[
                    1
                ]
            )
            ...
            try:
                prior_balance = (
                    transaction_complete.value.transaction.meta.post_balances[0]
                )
                initial_balance = (
                    transaction_complete.value.transaction.meta.pre_balances[0]
                )
                solana_account = transaction_complete.value.transaction.transaction.message.account_keys[
                    0
                ]
                balance_change = abs(
                    float(prior_balance) - float(initial_balance)
                ) / float(1000000000)
